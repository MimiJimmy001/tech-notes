...
这个项目不是只训练一个 GAN。它同时覆盖了生成模型训练、四类图像增广、数据管线、语义质量过滤、异常检测评测和 Web 演示，重点解决的是“生成出来的图到底能不能用于训练”。
...
项目基于 Focus-StyleGAN 工业缺陷增广系统继续演进，最终交付物不是微调后的模型，而是一条可复现的数据生产与质量评估管线，以及一个标准化的 SFT 数据集。
