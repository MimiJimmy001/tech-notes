...
window.portfolioDataFallback = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "solution": [
        "基于 MVTec AD 的 ground-truth mask 提取缺陷结构，统一转换为可读的结构化描述。",
        "使用 Qwen-VL 将结构化标注改写为自然语言，并统一训练数据格式。",
        "训练 StyleGAN 作为合成数据源，串联标注、改写、MD5 去重、质量评分和版本化管理。",
        "设计真实缺陷、真实正常和合成图组成的盲测对照实验，并分析评分理由的词频分布。"
      ],
      "impact": [
    ...
    },
    {
      ...
      "summary": "面向 PCB 工程检验与报价场景，处理 Gerber、ODB++、PDF 等多源异构设计文件，为模型训练和检索构建标准化数据资产。",
      "challenge": [
        "不同设计文件的坐标系、单位和图层命名不统一，难以直接入库。",
        "字段缺失、重复和格式异常会直接影响模型输入质量。",
        "大批量 API 调用存在限流、中断和断点续传问题。"
      ],
      "solution": [
        "使用 Python 和 pandas 统一坐标系、单位与图层命名，沉淀可复用清洗规则集。",
        "搭建查重、字段检查和缺失值扫描的自动化校验框架，建立数据字典与版本管理机制。",
        "实现带指数退避重试和断点续传的批量脚本，并构建可检索的异常案例库。"
      ],
      "impact": [
        "支撑 1200+ 张图纸稳定入库。",
        "拦截并修正 400+ 项异常数据。",
        "归档 20+ 组典型异常样本，形成自动化校验与人工抽检的双层质检流程。"
      ],
      "highlight": "多源数据清洗与质量治理"
    }
  ],
  "notes": [
    {
    ...
    {
      "id": "pcb-data-cleaning",
      "number": "03",
      "category": "数据工程",
      "title": "PCB 多源文件清洗：统一坐标、单位和图层口径",
      "excerpt": "Gerber、ODB++、PDF 看起来都是设计文件，但对数据管线而言，它们代表不同的坐标系、单位体系和结构表达。",
      "date": "2026.05.18",
      "readTime": "8 分钟",
      "coverImage": "",
      "lead": "多源数据清洗最难的部分通常不是代码，而是先明确不同来源字段的业务含义。",
      "sections": [
        {
          "heading": "数据问题",
          "content": "PCB 检验和报价需要处理 Gerber、ODB++、PDF 等多源文件。不同来源的坐标原点、单位精度和图层命名规则不统一，直接进入模型会影响后续检验和检索。"
        },
        {
          "heading": "清洗策略",
          "content": "使用 Python 和 pandas 统一坐标变换、单位换算与图层命名，并把不同来源的字段映射整理成可复用规则集。规则不是一次性脚本，而是和数据字典、采集规范一起版本化。"
        },
        {
          "heading": "自动校验",
          "content": "搭建覆盖查重、字段检查和缺失值扫描的自动化框架，在入库前拦截异常记录，再配合人工抽检处理无法自动判断的样本。"
        },
        {
          "heading": "工程结果",
          "content": "清洗流程支撑 1200+ 张图纸稳定入库，拦截并修正 400+ 项异常数据。实习期间归档 20+ 组典型异常案例，为后续模型迭代和问题回溯提供可检索样本。"
        }
      ]
    },
    {
